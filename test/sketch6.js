...
function draw() {
  ...
  angleX = map(deltaY, -height / 2, height / 2, -PI, PI);
  angleY = map(deltaX, -width / 2, width / 2, -PI, PI);
  ...
  push();
  rotateX(PI/2);
  translate(-taille/2, -taille/2, taille/2 - marg);
  image(img,0,0,taille,taille);
  pop();
